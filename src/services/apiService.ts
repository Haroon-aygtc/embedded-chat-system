...
export const chatApi = {
  sendMessage: async (message, contextRuleId) => {
    try {
      ...
      await sequelize.query(
        `INSERT INTO chat_messages (id, content, sender, context_rule_id, created_at) 
         VALUES (?, ?, ?, ?, ?)`,
        {
          replacements: [messageId, message, "user", contextRuleId, timestamp],
          type: sequelize.QueryTypes.INSERT,
        },
      );

      // Generate a response (in a real implementation, this would call an AI service)
      const responseId = uuidv4();
      const responseContent = `This is a response to your message: "${message}". In a production environment, this would be processed by an AI model.`;
  ...
  getHistory: async () => {
    try {
      ...
      return messages.map((msg) => ({
        ...
        status: "sent",
        contextRuleId: msg.context_rule_id,
      }));
    } catch (error) {
      logger.error(
  ...
  getContextHistory: async (contextRuleId) => {
    try {
      const sequelize = await getMySQLClient();
      const messages = await sequelize.query(
        `SELECT * FROM chat_messages WHERE context_rule_id = ? ORDER BY created_at DESC LIMIT 50`,
        {

...
class MySQLScrappingService {
  ...
  async startScraping(options: ScrapeOptions): Promise<string> {
    ...
    // Store job in MySQL database
    try {
      const sequelize = await getMySQLClient();
      await sequelize.query(
        `INSERT INTO scrape_jobs (id, url, timestamp, status, progress, data, metadata) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        {
          replacements: [
            jobId,
      ...
      logger.error(`Error storing scrape job ${jobId} in database:`, error);
      // Continue with in-memory job even if database storage fails
    }
  ...
  async getJobStatus(jobId: string): Promise<ScrapeResult | null> {
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        const result: ScrapeResult = {
          ...
          aiAnalysis: job.ai_analysis
            ? typeof job.ai_analysis === "string"
              ? JSON.parse(job.ai_analysis)
              : job.ai_analysis
            : undefined,
        };
        return result;
      ...
      logger.error(`Error fetching scrape job ${jobId} from database:`, error);
      return null;
    }
  }
  ...
  async getAllJobs(): Promise<ScrapeResult[]> {
    // Combine in-memory jobs with database jobs
    const memoryJobs = Array.from(this.activeJobs.values());

    try {
      const sequelize = await getMySQLClient();
      const [results] = await sequelize.query(
        `SELECT * FROM scrape_jobs ORDER BY timestamp DESC LIMIT 100`,
      );

      const dbJobs = (results as any[]).map((job) => ({
        ...job,
        data: typeof job.data === "string" ? JSON.parse(job.data) : job.data,
        metadata:
          typeof job.metadata === "string"
      ...
      // Merge the lists, prioritizing memory jobs
      const memoryJobIds = new Set(memoryJobs.map((job) => job.id));
      const filteredDbJobs = dbJobs.filter((job) => !memoryJobIds.has(job.id));

      return [...memoryJobs, ...filteredDbJobs];
    } catch (error) {
      logger.error("Error fetching scrape jobs from database:", error);
      // Return just the in-memory jobs if database fetch fails
      return memoryJobs;
    }
  }
   ...
   */
  private async updateJobInDatabase(job: ScrapeResult): Promise<void> {
    try {
      const sequelize = await getMySQLClient();
      await sequelize.query(
         ...
         WHERE id = ?`,
        {
          replacements: [
            ...
            job.aiAnalysis ? JSON.stringify(job.aiAnalysis) : null,
            job.error || null,
            job.exportPath || null,
            job.vectorStoreUrl || null,
            job.id,
          ],
        },
      );
    } catch (error) {
      logger.error(`Error updating scrape job ${job.id} in database:`, error);
      // Continue with in-memory job even if database update fails
    }
  }
  ...
  private async createVectorEmbeddings(job: ScrapeResult): Promise<string[]> {
    try {
      ...
      for (const chunk of chunks) {
        try {
          ...
          try {
            const sequelize = await getMySQLClient();
            await sequelize.query(
              `INSERT INTO scrape_vectors (id, content, metadata, embedding, job_id) 
               VALUES (?, ?, ?, ?, ?)`,
              {
                replacements: [
                  record.id,
            ...
            logger.error(
              "Error storing vector embedding in database:",
              dbError,
            );
          }
        } catch (e) {
      ...
      if (vectorIds.length > 0) {
        job.vectorStoreUrl = `/api/scrape/vectors/${job.id}`;
      }
  ...
  private async generateDataset(
    ...
    try {
      ...
      // Store dataset metadata in MySQL
      try {
        const sequelize = await getMySQLClient();
        await sequelize.query(
          `INSERT INTO scrape_datasets (id, name, description, source_url, created_at, item_count, metadata) 
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          {
            replacements: [
              datasetId,
              ...
              datasetMetadata.created_at,
              datasetMetadata.item_count,
              JSON.stringify(datasetMetadata.metadata),
            ],
          },
        );
        ...
        for (let i = 0; i < datasetItems.length; i += batchSize) {
          ...
          await sequelize.query(
            `INSERT INTO scrape_dataset_items (id, dataset_id, content, item_index, metadata) 
             VALUES ?`,
            {
              replacements: [batchValues],
        ...
        logger.error("Error storing dataset in database:", dbError);
        return undefined;
      }
    } catch (error) {
  ...
  private async exportData(
    ...
    try {
      ...
      job.exportPath = `/api/scrape/exports/${datasetName}.${exportOptions.format}`;
      ...
      if (exportOptions.saveToPublic) {
        try {
          ...
          const sequelize = await getMySQLClient();
          await sequelize.query(
            `INSERT INTO scrape_exports (id, job_id, name, format, content, created_at) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            {
              replacements: [
                uuidv4(),
          ...
          logger.error("Error exporting to database:", e);
        }
      }
    } catch (error) {
  ...
  async deleteJob(jobId: string): Promise<boolean> {
    ...
    const memoryResult = this.activeJobs.delete(jobId);

    // Also remove from database
    try {
      const sequelize = await getMySQLClient();
      await sequelize.query(`DELETE FROM scrape_jobs WHERE id = ?`, {
        replacements: [jobId],
      });
      return true;
    } catch (error) {
      logger.error(`Error deleting scrape job ${jobId} from database:`, error);
      // Return true if it was in memory, even if database delete failed
      return memoryResult;
    }
  }
  ...
  async searchVectorStore(query: string, limit: number = 5): Promise<any[]> {
    try {
      ...
      // Search MySQL vector store using cosine similarity
      // This is a simplified approach - in production you'd use a proper vector search
      const sequelize = await getMySQLClient();
      const [results] = await sequelize.query(
        `SELECT v.id, v.content, v.metadata, j.url, j.metadata as job_metadata 
         FROM scrape_vectors v 
         JOIN scrape_jobs j ON v.job_id = j.id 
         ORDER BY RAND() LIMIT ?`, // In a real implementation, you'd use vector similarity
        { replacements: [limit] },
      );
  ...
  async getDataset(datasetId: string): Promise<any> {
    try {
      ...
      const [metadataResults] = await sequelize.query(
        `SELECT * FROM scrape_datasets WHERE id = ?`,
      ...
      const [itemsResults] = await sequelize.query(
        `SELECT * FROM scrape_dataset_items WHERE dataset_id = ? ORDER BY item_index ASC`,
        { replacements: [datasetId] },
      );

      const items = (itemsResults as any[]).map((item) => ({
        ...item,
        metadata:
...
const mysqlScrappingService = new MySQLScrappingService();

export default mysqlScrappingService;

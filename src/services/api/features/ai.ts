...
import { apiKeyService } from "../utils/apiKeys";
...
class AIService {
   ...
   */
  async applyContextRuleToPrompt(
    query: string,
   ...
   */
  async generateGeminiResponse(prompt: string): Promise<AIModelResponse> {
   ...
   */
  async generateHuggingFaceResponse(prompt: string): Promise<AIModelResponse> {
    // Check if we have a cached response
    const cachedResponse = await aiCacheService.getCachedResponse(prompt);
  ...
  async generateResponse(
    ...
    const primaryModel = this.determineModel(query, contextRule);

    // Apply context rule to the prompt, including knowledge base integration
    const prompt = await this.applyContextRuleToPrompt(
      query,
      contextRule,
      promptTemplate,
      userId,
    ...
    try {
      ...
      // Apply context rule filtering to the response
      response.content = this.applyContextRuleToResponse(
        response.content,
        contextRule,
      );
      ...
      try {
        ...
        if (fallbackModel === "gemini") {
          fallbackResponse = await this.generateGeminiResponse(prompt);
        } else {
          fallbackResponse = await this.generateHuggingFaceResponse(prompt);
        }

        // Apply context rule filtering to the response
        fallbackResponse.content = this.applyContextRuleToResponse(
